define(['app', 'angular', 'baseService', 'baseDirective', 'pageDirective', 'jquery', 'uiBootstrap', 'bootstrapTable', 'math'],
    function (webapp) {//主控制器
        webapp
            .controller('baseController', function ($scope, baseService, $interval, $uibModal) {
                ...
                $scope.addExaminationData = function () {
                    var ModalDemoCtrl = function ($scope, $uibModal) {
                        $scope.open = function () {
                            var modalInstance = $uibModal.open({
                                templateUrl: './app/pages/addExaminationDataPage.html',
                                controller: "addExaminationDataPageCtrl",
                                size: "lg",
                                resolve: {
                                    // items: function () {
                                    //     return $scope.items;
                            ...
                            modalInstance.result.then(function (result) {
                                console.log(result);
                                var promise = baseService.addExaminationData(result);
                                promise.then(function (value) {
                                    if (value.success) {//刷新关系图
                                        $scope.getTypeMap();
                                    } else {
            ...
            .controller('addExaminationDataPageCtrl', function ($scope, baseService, $interval, $uibModalInstance, $timeout) {
                $scope.model = {};
                $scope.years = [];//年份数据
                var initDate = new Date();
                for (var index = initDate.getFullYear() - 20; index <= initDate.getFullYear(); index++) {
                ...
                $scope.chooseType = function (type, item) {
                    for (var index = type.level + 1; index <= $scope.maxLevel; index++) {
                        item["types_" + index] = [];
                        item["type" + index + "_id"] = undefined;
                    }
                    item["type" + type.level + "_id"] = type.id;
                ...
                $scope.getNextTypes = function (currentLevel, item) {
                    var level = currentLevel + 1;
                    var params = {
                        parent_id: item["type" + currentLevel + "_id"],
                        level: level
                    };
                    var promise = baseService.getNextTypes(params);
                    promise.then(function (data) {
                        if (data.success) {
                            item["types_" + level] = data.types;
                            angular.forEach($scope.children, function (tmp_item, tmp_item_index) {
                                if (item.id == tmp_item.id) {
                                    $scope.children[tmp_item_index] = item;
                                }
                            });
                        }
                    });
